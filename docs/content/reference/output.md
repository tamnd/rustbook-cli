...
describes what users get. Pick a format with `-o`, or let rustbook choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
rustbook <command> -o table   # aligned columns for reading
rustbook <command> -o jsonl   # one JSON object per line, for piping
rustbook <command> -o json    # a single JSON array
rustbook <command> -o csv     # spreadsheet friendly
rustbook <command> -o tsv     # tab-separated
rustbook <command> -o url     # just the URL column
rustbook <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
rustbook <command> --fields id,title,url
```
...
```bash
rustbook <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
rustbook <command>            # a table, because this is a terminal
rustbook <command> | wc -l    # JSONL, because this is a pipe
```
